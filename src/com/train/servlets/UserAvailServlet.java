...
public class UserAvailServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			RequestDispatcher rd = req.getRequestDispatcher("SeatAvailability.html");
			rd.include(req, res);

			if (train != null) {

				// Available Seats Highlight
				pw.println("<div class='main'>" + "<p1 class='menu'>Available Seats are " + "<span class='red'>"
						+ train.getSeats() + " Seats</span>" + "</p1>" + "</div>");

				// Horizontal Train Table
